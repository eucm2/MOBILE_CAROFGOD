    ...
    if (db.version == "1.0" || db.version == "") {
        db.changeVersion(db.version, "1.1", function (tx) {
            db.transaction(function (tx) {
                tx.executeSql("ALTER TABLE AUTOS ADD nota text;");
                tx.executeSql("ALTER TABLE AUTOS ADD porEliminar INTEGER NOT NULL DEFAULT '0';");
            }, error);
        }, error);
    } else if (db.version == "1.1" || db.version == "") {
        db.changeVersion(db.version, "1.2", function (tx) {
            db.transaction(function (tx) {
                tx.executeSql("ALTER TABLE AUTOS ADD jrac_crop_x REAL;");
                tx.executeSql("ALTER TABLE AUTOS ADD jrac_crop_y REAL;");
                tx.executeSql("ALTER TABLE AUTOS ADD jrac_crop_width REAL;");
                tx.executeSql("ALTER TABLE AUTOS ADD jrac_crop_height REAL;");
                tx.executeSql("ALTER TABLE AUTOS ADD jrac_image_width REAL;");
                tx.executeSql("ALTER TABLE AUTOS ADD jrac_image_height REAL;");
                tx.executeSql("ALTER TABLE AUTOS ADD rotar INTEGER;");
                tx.executeSql("ALTER TABLE AUTOS ADD escala INTEGER;");
            }, error);
        }, error);
    }
    db.transaction(crearTabla, error, llena);
    pictureSource = navigator.camera.PictureSourceType;
    destinationType = navigator.camera.DestinationType;
    //LLENA LOS CAMPOS DE CONFIGURACION
    //var db = window.openDatabase("carofgod", "1.0", "AUTOS PhoneGap", 200000);
    db.transaction(function (tx) {
...
$(document).on("click", "#tomarfoto", function () {
    if(device.platform=="Generic"){
        var imageURL=prompt("Url de su imagen","http://localhost/images/cayo.jpg");
...
$(document).on("click", "#selecfoto", function () {
    if(device.platform=="Generic"){
        var imageURL=prompt("Url de su imagen","http://i.imgur.com/bDYJ0ui.jpg");
        exitoTomaFoto(imageURL);
    }
    else{
...
//SUBIR FOTO
$(document).on("change", ".seleFotos", function () {
//$(document).on("click", ".botonSubir", function () {
    ...
    $.ajax({
        ...
        success: function (userJoomla) {
            ...
            if (idUserJoomla > 0) {
                ...
                ft.upload(urlImg, $("#web").val() + "/crm/recibefoto.php", function () {
                    $.ajax({
                        type: "GET",
                        url: requerimiento,
                        success: function (res) {
                ...
                <li id="--' + p.id + '" ' + subido + ' >\n\
                  <div class="ui-grid-a" >\n\
                    <div class="left-li ui-block-a">\n\
                      <img src="' + p.urlfoto + '" style="height:80px; width:80px;" />\n\
                    </div>\n\
                    <div class="right-li ui-block-b">\n\
                      <select data-transition="none" class="seleFotos" urlImg="' + p.urlfoto + '" idPadre="' + idAuto + '" idFoto="' + p.id + '" >\n\
                        <option value="1">Action</option>\n\
